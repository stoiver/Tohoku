...
def I5(xi, eta, q, dip, nu, R, db):
    ...
    if np.cos(dip) > eps:
        ...
        #                (xi*(R + X) * np.cos(dip)) ) 
        #I[xi == 0] = 0
        II = (eta * (X + q*np.cos(dip)) + X*(R + X) * np.sin(dip))
        
        I = II*0.0
    
        k = (xi != 0) 
        I[k] = (1 - 2 * nu) * 2 / np.cos(dip) * \
              np.arctan( II[k]/(xi[k]*(R[k] + X[k]) * np.cos(dip)) )
    else:
...
def plot_los(x, y, ux, uy, uz, los, params, profile=False):
    ...
    # --------------
    fig, (ax,ax1,ax2) = plt.subplots(1,3,subplot_kw=dict(aspect=1.0, adjustable='box'),sharex=True, sharey=True, figsize=(17,6))
